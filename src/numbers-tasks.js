...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstCathet = Math.abs(x1) + Math.abs(x2);
  const secondCathet = Math.abs(y1) + Math.abs(y2);
  const hypotenuseSquared = firstCathet ** 2 + secondCathet ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstAction = x1 * x2 + y1 * y2;
  const secondAction = Math.sqrt(x1 ** 2 + y1 ** 2);
  const thirdAction = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = firstAction / (secondAction * thirdAction);
...
function roundToPowerOfTen(num, pow) {
  const multiplier = Number(`1${'0'.repeat(pow)}`);
